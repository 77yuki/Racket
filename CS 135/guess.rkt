             ...
             [else (cond [(member? (first all) final)
                         ...
                         [else
                          (collect-attributes/final
                           (rest all)
                           (append final (list (first all))))])]))]
    (collect-attributes/final
     (all-attributes examples) original-attributes)))
...
(define (histogram examples)
  (local
    ...
    (histogram/att (all-attributes examples)
...
(define (augment-histogram histogram attributes total)
  (cond [(empty? attributes) empty]
        [(empty? histogram) (cons (list (first attributes) 0 total)
                                  (augment-histogram
                                   histogram (rest attributes) total))]
        [else (cond [(equal? (first attributes) (first (first histogram)))
                     (cons (list (first (first histogram))
                           ...
                           (augment-histogram
                            (rest histogram) (rest attributes) total))]
...
(check-expect (augment-histogram (list (list 'b 24) (list 'c 30) (list 'd 67))
                                 (list 'a 'b 'c 'd) 100)
              (list (list 'a 0 100) (list 'b 24 76)
                    (list 'c 30 70) (list 'd 67 33)))
                  ...
                  (define (remove-element/one one sym)
                     (cond [(empty? one) empty]
                           [(equal? (first one) sym)
                            (rest one)]
                           [else (cons (first one)
                                       (remove-element/one (rest one) sym))]))
                  (define (remove-element currentlists sym)
                    (cond [(empty? currentlists) empty]
                          [else
                           (cons (remove-element/one (first currentlists) sym)
                                 (remove-element (rest currentlists) sym))]))
                  (define list-of-with-current
                    (remove-element list-of-with root-attribute))]
...
;; i)
;;
;; (train-classifier examples label) generates a decision tree from the
;;   examples, and then produce a predicate that consumes a list of
;;   attributes and produces a decision according to label and what the
;;   question states
;; Examples:
